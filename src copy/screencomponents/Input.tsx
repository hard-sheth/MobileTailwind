...
// export default function Screeninput (propsFunction:any) {
//   const { inputType, label, secured, maxDate, minDate, name, value, onChange } = propsFunction;
...
const Screeninput = forwardRef((propsFunction:unknown|any, ref) => {
  const { inputType, label, secured, maxDate, minDate, name, value, onChange, errormessage } = propsFunction;
  const isDarkMode = useColorScheme() === 'dark';
  const [isDatePickerVisible, setDatePickerVisibility] = useState(false);
  const [selectedDate, setSelectedDate] = useState<Date | null | string>(value);
  const [inputValue, setInputValue] = useState(value);
...
console.log(errormessage,' errormessage');
  ...
  return (
    <View className={`mx-3 my-1`}>
      ...
      {inputType === 'date' && (
        <View className={`bg-slate-100 rounded-md px-2 h-12`}>
          <TouchableWithoutFeedback onPress={showDatePicker}>
            <View>
              <Text className={`text-sm py-3`}>{selectedDate ? selectedDate : ''}</Text>
            </View>
      ...
      {
        errormessage && <Text className={`text-red-600 mb-2 text-sm`}>{errormessage}</Text>
      }
    </View>
  );
});
